...
export default class JStageGroup extends JSubStage {
  ...
  _fechHalfWeeksSchedule: TypeHalfWeekOfYear[];
	...
	constructor(isgc: IJStageGroupCreator) {
		...
		this._groupsNumber = isgc.info.groupsNumber;
		...
		for (let i = 0; i < isgc.info.groupsNumber; i++) {
			this._groups.push(
				new JLeague({
					...isgc.info,
					temp: this.tournamentConfig.season,
					partsNumber: isgc.info.participantsPerGroup[i]
				})
			)
		}
	}
	...
	get relativeTable(): ITeamTableItem[] {
