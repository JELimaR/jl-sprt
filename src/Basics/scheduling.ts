...
export default (n: number, isIV: boolean): arr2<number>[][] => {
	...
	let out: arr2<number>[][] = mFuncObj[n-2]();

	if (isIV) {
		let out2: arr2<number>[][] = mFuncObj[n-2]();
		for (let f=0; f<out2.length;f++) {
			for (let m=0; m<out2[f].length;m++) {
				out2[f][m] = inverse(out2[f][m]);
			}
			out.push(out2[f]);
		}
	} else {
		for (let f=0; f<out.length;f++) {
			for (let m=0; m<out[f].length;m++) {
				out[f][m] = neutral(out[f][m]);
			}
		}
	}
	return out;
...
const inverse = (a: arr2<number>): arr2<number> => {
	return [a[1], a[0]];
}
